...
const calculateMean = values =>
  values.reduce((total, item) => total + item) / values.length;
const calculateStandardDeviation = values =>
  Math.sqrt(
    values
      .map(value => Math.pow(value - calculateMean(values), 2))
      .reduce((total, x) => total + x) / values.length,
  );
...
const App = () => {
  ...
  const search = async () => {
    const results = await fetch(
      `http://www.omdbapi.com/?apikey=${apiKey}&t=${query}&type=movie`,
    );
  ...
  if (currentlySelectedMovies.length > 0) {
    ...
    const boxOfficeArray = currentlySelectedMovies.map(currentlySelectedMovie =>
      parseInt(currentlySelectedMovie.BoxOffice.replace(/\$|,/g, '')),
    );
    ...
    <SafeAreaView style={styles.container}>
        ...
        {!searchError && newMovie && (
          <View style={styles.newMovie}>
            ...
            <TouchableOpacity
              onPress={() => {
                if (
                  currentlySelectedMovies.find(
                    movie => movie.imdbID === newMovie.imdbID,
                  )
                ) {
                  setSearchError(
                    'Movie already selected, please search for a new one',
                  );
                } else if (newMovie.BoxOffice === 'N/A') {
                  setSearchError(
                    'Movie does not contain box office data to display',
                  );
                  setNewMovie(null);
                } else {
                  setCurrentlySelectedMovies([
                    ...currentlySelectedMovies,
                    newMovie,
                  ]);
                  setNewMovie(null);
                  setQuery('');
      ...
      <View style={styles.dataContainer}>
        ...
        <View style={styles.dataPoint}>
          ...
          <Text style={{flex: 1}}>
            {mean.toLocaleString('en-US', {style: 'currency', currency: 'USD'})}
          </Text>
        </View>
        <View style={styles.dataPoint}>
          <Text style={{flex: 1}}>BO Standard Deviation</Text>
          <Text style={{flex: 1}}>
            {standardDeviation.toLocaleString('en-US', {
              style: 'currency',
              currency: 'USD',
            })}
          </Text>
